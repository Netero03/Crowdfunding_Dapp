...
contract CrowdFunding is Ownable {
    ...
    constructor(uint8 _platformTax,address initialOwner) Ownable(initialOwner) {
    ...
    function deleteCampaign(uint256 _id) external privilageEntity(_id) notInEmergency returns (bool) {
        ...
        if(campaigns[_id].amountCollected > 0 wei) {
         ...
         // Shift campaigns after the deleted one one position backward
        for (uint256 i = _id; i < numberOfCampaigns - 1; i++) {
            campaigns[i] = campaigns[i + 1];
        }
        delete campaigns[numberOfCampaigns - 1]; // Delete the last (now duplicate) element
        numberOfCampaigns -= 1;
